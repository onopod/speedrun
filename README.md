...
- Three.js / WebGLによるカーブ・起伏のある640mの3Dタイムアタックコース（RIVER RUN v4）
...
`DATABASE_URL`（未指定時はVercel連携の `POSTGRES_URL`）はサーバー専用。初回APIアクセスで `speedrun_guests_v2` / `speedrun_runs_v3` とインデックスを加算的に作成します（`src/lib/guest-store.ts`）。DDLはトランザクションのadvisory lockで同時起動を直列化し、失敗時は次回リクエストで再試行。既存テーブルは削除・変更しません。v3以降の記録の下限は25秒。ジャンプ仕様が変わったv4はコースIDを分け、旧版のランキングとは分離します。匿名ユーザー名は引き継ぎます。旧v2/v3の未送信キューも端末に保持しますが、新コースへは送信しません。DBロールにCREATE権限が必要です。
