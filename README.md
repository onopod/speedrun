...
- Three.js / WebGLによるカーブ・起伏のある640mの3Dタイムアタックコース（RIVER RUN v3）
...
タッチ端末は画面の左右スワイプで横移動し、左右ボタンは表示しません。画面幅の65%のスライドでコース幅を移動し、指を離すとその位置を維持します。ジャンプ・加速ボタンは残し、スワイプしながら別の指で押せます。開始後6.5秒は半透明の矢印と指のガイドを表示します。通常速度15.6、加速20.8、減速13、アイテム加速23.4（すべて旧版の1.3倍）。ジャンプ初速13、上昇中の重力22、落下中30。速度と加速度から位置を計算し、頂点をまたぐステップを分割するため滑らかに減速・加速します。最高点は約3.8m。物理は120Hz固定ステップ、タブ非表示中と名前編集中は停止します。
...
`DATABASE_URL`（未指定時はVercel連携の `POSTGRES_URL`）はサーバー専用。初回APIアクセスで `speedrun_guests_v2` / `speedrun_runs_v3` とインデックスを加算的に作成します（`src/lib/guest-store.ts`）。DDLはトランザクションのadvisory lockで同時起動を直列化し、失敗時は次回リクエストで再試行。既存テーブルは削除・変更しません。v3は速度変更に合わせて記録の下限を25秒とし、旧版のランキングとは分離します。匿名ユーザー名は引き継ぎます。旧v2の未送信キューも端末に保持しますが、新コースへは送信しません。DBロールにCREATE権限が必要です。
