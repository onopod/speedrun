...
import { CLASSIC_COURSE, findCourse } from "@/lib/courses";

export const runtime = "nodejs";
const headers = { "Cache-Control": "no-store" };
export async function GET(request: Request) {
  const course = findCourse(new URL(request.url).searchParams.get("course") ?? CLASSIC_COURSE.id);
  if (!course) return NextResponse.json({ error: "Unknown course" }, { status: 400, headers });
  try {
    ...
    const result = await db.execute(sql`SELECT best.id, g.name, best.time_ms AS "timeMs", best.coins,
      ...
      FROM (SELECT DISTINCT ON (guest_id) * FROM speedrun_runs_v3 WHERE course = ${course.id}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await db.execute(sql`INSERT INTO speedrun_runs_v3(id, guest_id, course, time_ms, coins, input_type)
      VALUES (${score.runId}, ${guest.id}, ${score.course}, ${score.timeMs}, ${score.coins}, ${score.input})
      ON CONFLICT (id) DO UPDATE SET id = EXCLUDED.id
      WHERE speedrun_runs_v3.guest_id = EXCLUDED.guest_id RETURNING id`);
