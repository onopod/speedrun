...
import { COURSE_LENGTH, RUN_SPEED, finishGrade } from "@/lib/autorun";
...
export default function SpeedrunGame() {
    ...
    if (mount.current) {
      try {
        ...
        try {
          ...
          const variant: RunnerVariant = saved.character === "hikari" ? "hikari" : "hayate", guide = saved.guide !== false, mute = saved.muted === true;
  ...
  return <section className={`game-shell ${hud.boost ? "boosting" : ""}`}>
    ...
    {showGuide && hud.phase === "running" && !hud.paused && !editing && <div className={`route-hint ${hud.nextJump < 6 && hud.nextJump > -1 ? "jump-now" : ""}`} aria-label="回収ルートガイド"><span>◇ STAR LINE</span><b>{hud.nextJump < 6 && hud.nextJump > -1 ? "ここで長押しジャンプ！" : hud.nextJump < 25 ? `大ジャンプまで ${Math.max(0, Math.round(hud.nextJump))} m` : "水色のラインで全スターを目指そう"}</b><small>通常速度・大ジャンプの回収ルート</small></div>}
