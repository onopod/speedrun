...
export function ensureGuestSchema() {
  schemaReady ??= db.transaction(async tx => {
    ...
    await tx.execute(sql`CREATE TABLE IF NOT EXISTS speedrun_runs_v3 (
      id UUID PRIMARY KEY, guest_id UUID NOT NULL REFERENCES speedrun_guests_v2(id),
      course VARCHAR(40) NOT NULL, time_ms INTEGER NOT NULL CHECK(time_ms BETWEEN 25000 AND 3600000),
      coins INTEGER NOT NULL CHECK(coins BETWEEN 0 AND 60), input_type VARCHAR(16) NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )`);
    await tx.execute(sql`CREATE INDEX IF NOT EXISTS speedrun_runs_v3_course_time_idx ON speedrun_runs_v3(course, time_ms, created_at)`);
    await tx.execute(sql`CREATE INDEX IF NOT EXISTS speedrun_runs_v3_guest_idx ON speedrun_runs_v3(guest_id, course)`);
